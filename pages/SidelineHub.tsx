...
import { 
    ...
    WhistleIcon, WifiOffIcon, ClockIcon,
    PenIcon, ChevronDownIcon, XIcon, SaveIcon, ActivityIcon,
    TrendingUpIcon, TargetIcon, TrashIcon
            ...
            <div className="grid grid-cols-3 gap-2">
                {actions.map(a => (
                    <button 
                        ...
                        className={`py-4 rounded-xl text-[10px] font-black uppercase transition-all border-2 ${playType === a.id && (a.res ? playResult === a.res : true) ? 'bg-highlight border-highlight text-white shadow-glow' : 'bg-black/20 border-white/5 text-text-secondary hover:border-white/20'}`}
                    >
                        {a.l}
                    </button>
                ))}
    ...
    return (
        <div className="flex flex-col lg:flex-row h-[calc(100vh-5rem)] gap-4 animate-fade-in overflow-hidden p-2">
            ...
            <div className="w-full lg:w-[420px] shrink-0 flex flex-col gap-3">
                <div className="bg-secondary/60 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-6 flex flex-col shadow-2xl h-full">
                    
                    {/* Unidade & Clock */}
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex bg-black/40 p-1 rounded-xl border border-white/5">
                            {['OFFENSE', 'DEFENSE', 'ST'].map(u => (
                                <button key={u} onClick={() => { setActiveUnit(u as any); resetConsole(); }} className={`px-4 py-1.5 rounded-lg text-[9px] font-black transition-all ${activeUnit === u ? 'bg-highlight text-white' : 'text-text-secondary'}`}>{u === 'ST' ? 'ST' : u === 'OFFENSE' ? 'ATK' : 'DEF'}</button>
                            ))}
                        </div>
                        <div className="bg-black/60 px-3 py-1.5 rounded-xl border border-highlight/20 font-mono text-highlight text-sm font-bold">
                            {activeGame.clock}
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 flex-1">
                        {/* 1. Ações Rápidas */}
                        <section>
                            <p className="text-[9px] font-black text-text-secondary uppercase tracking-widest mb-3 ml-1">1. Selecione a Jogada</p>
                            {renderActionGrid()}
                        </section>

                        {/* 2. Cluster Numérico */}
                        <section className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                            <div>
                                <label className="text-[9px] font-black text-text-secondary uppercase mb-2 block ml-1">2. Atleta (#)</label>
                                <div className="relative">
                                    ...
                                    />
                                    {selectedPlayer && (
                                        <div className="absolute right-2 top-2 bottom-2 bg-highlight/20 rounded-xl px-2 flex items-center border border-highlight/30">
                                            <LazyImage src={selectedPlayer.avatarUrl} className="w-8 h-8 rounded-full" />
                                        </div>
                                    )}
                                </div>
                            </div>
                            <div>
                                <label className="text-[9px] font-black text-text-secondary uppercase mb-2 block ml-1">3. Jardas (Yds)</label>
                                <div className="flex gap-2">
                                    <button onClick={() => setYardSide(yardSide === '+' ? '-' : '+')} className={`w-12 rounded-2xl font-black text-xl border-2 transition-all ${yardSide === '+' ? 'bg-green-600/20 border-green-500 text-green-400' : 'bg-red-600/20 border-red-500 text-red-400'}`}>
                                        {yardSide}
                                    </button>
                                    <input 
                                        type="number"
                    ...
                    <button 
                        ...
                        disabled={!playType}
                        className="w-full bg-highlight hover:bg-highlight-hover text-white font-black py-6 rounded-[2rem] uppercase italic text-xl shadow-glow transform active:scale-95 transition-all disabled:opacity-10 mt-6"
                    >
                        ARQUIVAR SNAP
                    </button>
                </div>
                ...
                <div className="bg-black/20 p-4 rounded-3xl border border-white/5 flex justify-between items-center px-6">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[8px] font-black text-text-secondary uppercase tracking-widest">{isOnline ? 'Sincronizado' : 'Modo Offline'}</span>
                    </div>
                    <span className="text-[8px] font-black text-text-secondary uppercase">Session: Q{activeGame.currentQuarter}</span>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-3 min-h-0">
                <div className="flex justify-between items-center px-4 shrink-0">
                    <h3 className="text-[10px] font-black text-white uppercase italic tracking-[0.4em] flex items-center gap-2">
                        <ClockIcon className="w-4 h-4 text-highlight" /> Log de Transmissão
                    </h3>
                </div>
