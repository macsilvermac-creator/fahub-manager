...
const Register: React.FC = () => {
  ...
  /* Fix: Added cpf to formData state to match register service requirements */
  const [formData, setFormData] = useState({ name: '', email: '', password: '', role: 'MASTER' as UserRole, cpf: '' });
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      /* Fix: Added missing cpf argument (5th parameter) to authService.register call as required by its definition */
      await authService.register(formData.name, formData.email, formData.role, formData.password, formData.cpf);
      setSuccess(true);
    ...
    <div className="min-h-screen flex items-center justify-center bg-primary relative overflow-hidden">
      <div className="bg-secondary/70 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-white/10 w-full max-w-md relative z-10 animate-fade-in">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
            ...
            {/* Fix: Added CPF input field to registration form as it is required by the auth service */}
            <div>
