...
const Academy: React.FC = () => {
    ...
    // Rotação da Loja com Proteção Anti-Crash
    useEffect(() => {
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20">
            <PageHeader title="Elite Academy" subtitle="Evolução técnica fora das 100 jardas." />
            ...
            {view === 'COURSES' ? (
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-24 bg-gradient-to-b from-indigo-950/30 to-black rounded-[3rem] border-2 border-indigo-500/20 p-6 flex flex-col gap-8 shadow-2xl min-h-[600px]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-slide-in">
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        <button onClick={() => setLocation('GYM')} className={`p-3 rounded-2xl border text-[8px] font-black transition-all ${location === 'GYM' ? 'bg-orange-600 border-orange-500 text-white' : 'bg-black/20 border-white/5 text-text-secondary'}`}>ACADEMIA</button>
                                        <button onClick={() => setLocation('HOME')} className={`p-3 rounded-2xl border text-[8px] font-black transition-all ${location === 'HOME' ? 'bg-orange-600 border-orange-500 text-white' : 'bg-black/20 border-white/5 text-text-secondary'}`}>CASA</button>
                                        <button onClick={() => setLocation('CALISTHENICS')} className={`p-3 rounded-2xl border text-[8px] font-black transition-all ${location === 'CALISTHENICS' ? 'bg-orange-600 border-orange-500 text-white' : 'bg-black/20 border-white/5 text-text-secondary'}`}>RUAS</button>
                                    </div>
                                </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        {workout ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center opacity-10 py-40 border-2 border-dashed border-white/10 rounded-3xl">
                                <DumbbellIcon className="w-20 h-20 mb-4" />
                                <p className="font-black uppercase tracking-widest text-sm text-center">Defina os parâmetros para gerar o treino.</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <Modal isOpen={!!selectedCourse} onClose={() => setSelectedCourse(null)} title={selectedCourse?.title || "Aula"} maxWidth="max-w-6xl">
                 ...
                 <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                     ...
                     <div className="bg-white/5 rounded-3xl p-6 border border-white/5">
                         ...
                         <button className="w-full py-3 bg-highlight text-white rounded-xl font-black uppercase text-xs">Continuar Estudo</button>
                     </div>
                 </div>
            </Modal>
        </div>
