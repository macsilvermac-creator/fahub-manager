...
import { WorkoutProof, Player, GymDay } from '../types';
    ...
    const handleCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = async () => {
                ...
                // Validação Híbrida 1: IA analisa imagem
                toast.info("IA analisando ambiente de treino...");
                ...
                if (player) {
                    ...
                    if (isValidByAI) {
                        ...
                        toast.warning("IA não reconheceu a academia. Enviado para revisão humana.");
                    }
                }
    ...
    const handleCoachAction = (playerId: number, proofId: string, approve: boolean) => {
        ...
        const updated = players.map(p => {
            if (p.id === playerId) {
                ...
                if (approve) p.xp += 50; // XP apenas se aprovado
        ...
        loadValidationQueue();
        toast.success(approve ? "Treino validado!" : "Treino rejeitado.");
    };
        ...
        <div className="space-y-6 pb-12 animate-fade-in">
            <div className="flex justify-between items-center bg-secondary p-4 rounded-2xl border border-white/5">
                ...
                <div className="flex bg-black/40 p-1 rounded-xl">
                    <button onClick={() => setViewMode('CATALOG')} className={`px-4 py-2 rounded-lg text-xs font-bold ${viewMode === 'CATALOG' ? 'bg-highlight text-white' : 'text-text-secondary'}`}>Conteúdo</button>
                    <button onClick={() => setViewMode('GYM')} className={`px-4 py-2 rounded-lg text-xs font-bold ${viewMode === 'GYM' ? 'bg-highlight text-white' : 'text-text-secondary'}`}>Treino de Força</button>
                    {!isPlayer && (
                        <button onClick={() => setViewMode('PENDING')} className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 ${viewMode === 'PENDING' ? 'bg-red-600 text-white' : 'text-red-400'}`}>
                            Validação {coachQueue.length > 0 && <span className="bg-white text-red-600 w-4 h-4 rounded-full flex items-center justify-center text-[10px]">{coachQueue.length}</span>}
                        </button>
                    )}
                </div>
            </div>
            ...
            {viewMode === 'GYM' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-slide-in">
                    <div className="lg:col-span-1 space-y-6">
                        <Card title="Personal Trainer IA">
                            <div className="space-y-4">
                                <textarea 
                                    ...
                                    placeholder="Ex: Treino para DL focado em explosão e força bruta..."
                                    value={gymGoal}
                                ...
                                <button onClick={handleGenerateWorkout} disabled={isGeneratingWorkout} className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2">
                                    {isGeneratingWorkout ? 'Montando Série...' : <><SparklesIcon className="w-4 h-4"/> Criar Plano Customizado</>}
                                </button>
                                
                                {isPlayer && (
                                    <div className="pt-4 border-t border-white/5">
                                        <h4 className="text-xs font-bold text-text-secondary uppercase mb-3">Check-in de Academia (Prova Real)</h4>
                                        ...
                                        <p className="text-[9px] text-text-secondary mt-2 text-center">⚠ Câmera nativa apenas. Provas da galeria são rejeitadas automaticamente.</p>
                                    </div>
                                )}
                            </div>
                        </Card>
                    ...
                    <div className="lg:col-span-2">
                        {structuredWorkout ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center bg-secondary/30 rounded-3xl border border-dashed border-white/5 p-12 opacity-50">
                                ...
                                <p className="text-sm">Configure seu objetivo para receber o plano da IA.</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {viewMode === 'PENDING' && (
                <div className="space-y-4 animate-slide-in">
                    <h3 className="text-xl font-bold text-white">Fila de Aprovação (Treinos)</h3>
                            ...
                            <div key={idx} className="bg-secondary rounded-2xl overflow-hidden border border-white/10 flex flex-col shadow-lg">
                                <div className="h-64 bg-black relative">
                                    ...
                                    <div className={`absolute top-2 right-2 px-2 py-1 rounded text-[8px] font-black uppercase ${item.proof.aiValidation === 'VALID' ? 'bg-green-500 text-black' : 'bg-red-500 text-white'}`}>
                                        IA: {item.proof.aiValidation === 'VALID' ? 'VERIFICADO' : 'SUSPEITO'}
                                    </div>
                                </div>
