...
const CheckInModal: React.FC<CheckInModalProps> = ({ isOpen, onClose, session, allPlayers, onSave }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            // Se já houve check-in, carrega ele. Se não, pré-carrega quem deu RSVP 'Sim'
    ...
    const displayedPlayers = allPlayers.filter(p => {
        ...
        if (filter === 'RSVP') {
            // Show if RSVP'd OR if already marked present (even if didn't RSVP)
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Lista de Presença (Check-in)" maxWidth="max-w-2xl">
            <div className="space-y-4">
                ...
                {/* Player Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[400px] overflow-y-auto custom-scrollbar p-1">
                ...
                <div className="flex gap-3 pt-4 border-t border-white/10">
                    <button onClick={onClose} className="flex-1 py-3 rounded-xl text-text-secondary hover:text-white font-bold transition-colors">
                        Cancelar
                    </button>
                    <button onClick={handleSave} className="flex-[2] bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-xl shadow-lg flex items-center justify-center gap-2">
