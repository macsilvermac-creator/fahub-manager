...
const GameManagementModal: React.FC<GameManagementModalProps> = ({ isOpen, onClose, game, onSave }) => {
  ...
  const [gameRoster, setGameRoster] = useState<string[]>([]); // IDs dos jogadores relacionados
  ...
  useEffect(() => {
    if (isOpen && game) {
        ...
        // Inicializa Roster do Jogo (se já salvo ou todos ativos por padrão)
        // @ts-ignore - game.attendees não existe na tipagem original, usando lógica local ou mock
        const initialRoster = game['gameRoster'] || allPlayers.filter(p => p.status === 'ACTIVE').map(p => String(p.id));
  ...
  const handleSaveAll = () => {
      if(!game) return;
      // @ts-ignore - adicionando propriedade dinâmica para persistência simples
      const updatedGame = {
          ...game,
  ...
  const handleAiScout = async () => {
      ...
      setIsAnalyzing(true);
      try {
          // Simula contexto com base no nome do time
