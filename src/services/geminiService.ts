...
export const importPlaybookFromImage = async (base64: string): Promise<PlayElement[]> => {
    try {
        const ai = getClient();
        // Use gemini-2.5-flash-image for image processing
        const response = await ai.models.generateContent({
...
export const explainPlayImage = async (base64ImageData: string, question: string) => {
    try {
        const ai = getClient();
        const response = await ai.models.generateContent({
            model: "gemini-2.5-flash-image",
        ...
        return response.text || "";
    } catch (e) { return "Erro na análise de imagem."; }
};

export const generateColorCommentary = async (homeTeam: string, awayTeam: string, context: string) => {
    const prompt = `Generate color commentary for a football game: ${homeTeam} vs ${awayTeam}. Context: ${context}. Return JSON { intro, homePlayerToWatch, awayPlayerToWatch, keyMatchups }`;
