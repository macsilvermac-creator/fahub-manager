...
import { LayoutDashboard, Users, ShieldAlert, Calendar, ChevronRight, ClipboardList, BarChart3, Search } from 'lucide-react';
import { NexusModuleProps, PracticeReport } from './types';
import { HCService } from './service';

const HC_Module: React.FC<NexusModuleProps> = ({ entityId, supabaseClient, onAction }) => {
  const [view, setView] = useState<'dash' | 'reports' | 'intel' | 'roster' | 'schedule'>('dash');
  ...
  const Card = ({ title, icon: Icon, onClick, children }: any) => (
    <div 
      onClick={onClick}
      className="group cursor-pointer bg-[#0a0f1e]/40 border border-white/5 backdrop-blur-xl p-6 rounded-xl hover:border-blue-500/50 transition-all duration-500 animate-in fade-in fill-mode-both"
    >
  ...
  if (view === 'dash') {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 h-full">
        ...
        <Card title="Adversário" icon={BarChart3} onClick={() => setView('intel')}>
        ...
        <Card title="Atletas" icon={Users} onClick={() => setView('roster')}>
        ...
        <Card title="Calendário" icon={Calendar} onClick={() => setView('schedule')}>
  ...
  // VIEW: SUBPAGINA RELATÓRIOS (2/3 e 1/3)
  if (view === 'reports') {
    return (
      <div className="flex flex-col md:flex-row gap-4 p-4 h-full animate-in slide-in-from-right duration-500">
        <div className="w-full md:w-2/3 bg-[#0a0f1e]/60 border border-white/5 p-8 rounded-xl backdrop-blur-md">
          ...
          <h2 className="font-black italic uppercase tracking-tighter text-4xl mb-4">Relatório Detalhado</h2>
        ...
        <div className="w-full md:w-1/3 bg-black/20 border border-white/5 rounded-xl flex flex-col overflow-hidden">
          <div className="p-4 border-b border-white/5 bg-white/5 font-black italic uppercase text-xs">Biblioteca</div>
          <div className="overflow-y-auto flex-1 p-2 space-y-2">
            {reports.map((r) => (
              ...
              >
                ...
                <p className="text-xs font-black uppercase italic">{r.coordinator_name}</p>
                <p className="text-[10px] opacity-50">{r.unit}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="p-8 flex items-center justify-center h-full">
      <p className="font-black italic text-white/20 animate-pulse">CARREGANDO SUBMÓDULO NEXUS...</p>
      <button onClick={() => setView('dash')} className="ml-4 underline text-xs">Voltar</button>
    </div>
  );
};
