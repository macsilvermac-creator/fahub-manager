...
import type { NexusModuleProps, PracticeReport } from './types';
...
const HC_Module: React.FC<NexusModuleProps> = ({ entityId, supabaseClient }) => {
  ...
  useEffect(() => {
    ...
    if (supabaseClient && entityId) loadData();
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 h-full">
        <Card title="Relatórios" icon={ClipboardList} onClick={() => setView('reports')}>
          ...
          <p className="text-sm font-bold text-blue-400 mt-1">{reports[0]?.coordinator_name || 'Aguardando Dados...'}</p>
        </Card>

        <Card title="Adversário" icon={BarChart3} onClick={() => {}}>
          <p className="text-xs text-white/40">Próximo Matchup</p>
          <p className="text-sm font-bold text-red-400 mt-1 italic uppercase tracking-widest">Scouting Ativo</p>
        </Card>

        <Card title="Atletas" icon={Users} onClick={() => {}}>
          <div className="flex gap-4 mt-2">
            <div className="text-center"><p className="text-lg font-black italic text-white">45</p><p className="text-[10px] opacity-50 text-white">TOTAL</p></div>
        ...
        <Card title="Calendário" icon={Calendar} onClick={() => {}}>
          <div className="mt-2 border-l-2 border-blue-500 pl-3">
            <p className="text-xs font-bold italic text-white">DOMINGO - 15:00</p>
            <p className="text-sm opacity-80 uppercase text-white">Estádio Municipal</p>
          </div>
        </Card>
      </div>
    );
    ...
    return (
      <div className="flex flex-col md:flex-row gap-4 p-4 h-full animate-in slide-in-from-right duration-500">
        <div className="w-full md:w-2/3 bg-[#0a0f1e]/60 border border-white/5 p-8 rounded-xl backdrop-blur-md">
          <button onClick={() => setView('dash')} className="text-[10px] uppercase font-bold text-blue-400 mb-4 tracking-tighter hover:underline">← Voltar para Nexus</button>
        ...
        <div className="w-full md:w-1/3 bg-black/20 border border-white/5 rounded-xl flex flex-col overflow-hidden h-[600px]">
          <div className="p-4 border-b border-white/5 bg-white/5 font-black italic uppercase text-xs text-white">Biblioteca de Treinos</div>
          <div className="overflow-y-auto flex-1 p-2 space-y-2">
            {reports.map((r) => (
              ...
              >
                ...
                <p className="text-[10px] opacity-50 text-white">{r.unit}</p>
              </div>
            ))}
          </div>
        </div>
