...
import { 
  ...
  BarChart4, Sparkles, Database, ChevronRight
} from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-white font-sans overflow-hidden italic">
      ...
      <aside className="w-20 bg-[#0a0f1e] border-r border-white/5 flex flex-col items-center py-8 shrink-0">
        <button onClick={() => navigate('/comercial')} className="p-4 bg-white/5 rounded-2xl text-slate-500 hover:text-white transition-all mb-8">
          <ArrowLeft size={24} />
        </button>
      ...
      <main className="flex-1 flex gap-4 p-4">
        ...
        <div className="flex-[2] flex flex-col gap-4 overflow-hidden">
          {/* TELA DE ANÁLISE (Onde Jules responde) */}
          ...
          <div className="h-40 bg-[#0f172a] border border-blue-500/30 rounded-[2.5rem] p-6 flex items-center gap-4 shadow-2xl shadow-blue-900/10">
            <div className="flex-1 relative">
              <input 
                ...
                className="w-full bg-transparent border-none outline-none text-sm font-bold placeholder:text-slate-600 italic pr-12"
              />
        ...
        <div className="flex-1 bg-[#0a0f1e] border border-white/5 rounded-[2.5rem] flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
             {[1, 2, 3].map((i) => (
               <div key={i} className="p-4 bg-white/5 border border-white/5 rounded-2xl hover:border-blue-500/30 cursor-pointer transition-all group">
                  <p className="text-[8px] text-slate-500 font-bold uppercase mb-1">Consulta #00{i}</p>
                  <p className="text-[10px] font-black uppercase text-white group-hover:text-blue-400 italic">Análise de ROI: Mercado Têxtil</p>
      ...
      <JulesAgent context="COMMERCIAL_LAB" />
    </div>
  );
};
