 ...
 * Foco: Gestão de Material Humano e Emissão de Ordens de Ação.
 */
...
export default function HumanCapital() {
  ...
  // Estado para Gerenciamento de Membro (Drawer/Modal)
  ...
  const fetchMembers = async () => {
    setLoading(true);
    // Busca integrada nas tabelas de perfis/atletas
    const { data, error } = await supabase
      .from('athletes')
      .select('id, full_name, status, position'); // Adaptar colunas conforme schema real
    
    if (!error && data) {
      // Mapeamento temporário para categorias do HumanCapital
      const mapped = data.map((m: any) => ({
        id: m.id,
        full_name: m.full_name,
        role: 'ATLETA',
        category: 'ELITE',
        status: m.status === 'active' ? 'ACTIVE' : 'EVALUATION',
        position: m.position
      }));
      setMembers(mapped);
    }
  ...
  const handleEmitAction = async () => {
    ...
    // Protocolo de Emissão de Ordem: Salva notificação para o alvo
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
      {/* HEADER OPERACIONAL - CLEAN STYLE */}
      <header className="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center sticky top-0 z-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Nexus Operational Module</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredMembers.map((member) => (
              <div key={member.id} className="bg-white border border-slate-200 rounded-3xl p-6 hover:shadow-xl hover:border-indigo-500/20 transition-all group">
                ...
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">{member.full_name}</h3>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">POS: {member.position || 'Não Definida'}</p>
      ...
      {/* DRAWER DE GESTÃO E RECOMENDAÇÃO */}
      {selectedMember && (
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="w-full max-w-md bg-white h-full shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col font-sans">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              ...
              {/* Módulo de Recomendação Técnica */}
                  ...
                  {[
                    ...
                    <button 
                      ...
                      className={`flex items-center gap-2 p-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                        target === t.id ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-200 text-slate-500 hover:border-indigo-300'
                      }`}
                    >
                      {t.icon} {t.label}
                    </button>
              ...
              <div className="space-y-4">
                ...
                <textarea 
                  className="w-full h-40 bg-slate-50 border border-slate-200 rounded-3xl p-5 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none font-medium text-slate-700 placeholder:text-slate-300 placeholder:italic"
                  placeholder="Ex: Recomendo transição para o elenco principal após o próximo tryout..."
