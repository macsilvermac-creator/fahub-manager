...
import { 
  ...
  ShieldCheckIcon, TrophyIcon, BookIcon
} from './icons/UiIcons';
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0F172A] transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-200 lg:relative lg:translate-x-0 flex flex-col h-full border-r border-white/5 shadow-2xl`}>
      ...
      <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar py-4 px-3">
        <nav className="flex-1">
          ...
          {securityService.canAccess(currentRole, 'SPORTS') && (
            <>
              <p className="px-4 text-[9px] font-black text-text-secondary/40 uppercase tracking-widest mt-6 mb-2 border-b border-white/5 pb-1">Gestão Esportiva</p>
              ...
              <NavLink to="/tactical-lab" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeNavLinkClasses : ''}`}>
                ...
                <span>Playbook {program}</span>
              </NavLink>

              {program === 'TACKLE' && (
                <NavLink to="/inventory" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeNavLinkClasses : ''}`}>
                  <ShieldCheckIcon className="w-4 h-4 mr-3 text-red-400" />
                  <span>Almoxarifado Pesado</span>
                </NavLink>
              )}
          ...
          {securityService.canAccess(currentRole, 'COMMERCIAL') && (
            <>
              <p className="px-4 text-[9px] font-black text-text-secondary/40 uppercase tracking-widest mt-6 mb-2 border-b border-white/5 pb-1">Administração</p>
          ...
          {securityService.canAccess(currentRole, 'SETTINGS') && (
            <>
              <p className="px-4 text-[9px] font-black text-text-secondary/40 uppercase tracking-widest mt-6 mb-2 border-b border-white/5 pb-1">Configurações</p>
      ...
      <div className="p-4 border-t border-white/5">
           <button onClick={() => authService.logout()} className="w-full py-2 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white rounded-lg text-[10px] font-black uppercase transition-all">Sair</button>
      </div>
    </aside>
  );
});
